...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          "Full design token palette — colors resolve from CSS variables so swatches update automatically when the theme changes.",
      },
    },
  },
} satisfies Meta
...
function RadiusSwatch({ label, variable, size }: { label: string; variable: string; size: string }) {
  return (
    <div className="flex flex-col gap-2">
      <div
...
export const Palette: Story = {
  render: () => (
    <div className="space-y-10 p-2">
      ...
      <Section title="Muted & Accent text">
        <Swatch bg="--muted-foreground" fg="--background" label="Muted FG" variable="--muted-foreground" />
        <Swatch bg="--accent-foreground" fg="--accent" label="Accent FG" variable="--accent-foreground" />
      </Section>
      ...
      <div className="space-y-3">
        <h3 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground border-b pb-2">
          Radius — 0.25rem base (sharp)
        </h3>
        <div className="grid grid-cols-3 gap-4 sm:grid-cols-4 md:grid-cols-6">
          <RadiusSwatch label="sm" variable="--radius-sm" size="×0.6" />
          <RadiusSwatch label="md" variable="--radius-md" size="×0.8" />
          <RadiusSwatch label="lg" variable="--radius-lg" size="base" />
          <RadiusSwatch label="xl" variable="--radius-xl" size="×1.4" />
          <RadiusSwatch label="2xl" variable="--radius-2xl" size="×1.8" />
          <RadiusSwatch label="3xl" variable="--radius-3xl" size="×2.2" />
        </div>
      </div>
    </div>
  ),
  ...
  render: () => (
    <div className="flex flex-wrap gap-4">
      {[
        ...
        <div
          key={c.bg}
          className="flex h-24 w-40 flex-col items-center justify-center rounded-2xl border shadow-xs gap-1"
    ...
    >
      ...
      >
        ...
        >
          ...
          <p className="text-sm">Surface depth demo</p>
        </div>
      </div>
    </div>
  ),
